...
double multiplyTransposedMatrix(int n){
	...
	secs = (double)(t_fin - t_ini) / CLOCKS_PER_SEC_1;
  	/*printf("La matriz traspuesta tardo %.16g milisegundos\n", secs * 1000.0);*/
...
int main(int argc, char** argv){
	...
	int tam = 0;
	int forma_de_operar = 0;
	double tiempo = 0.0;
	
	if (argc != 2){
		perror("Debe introducir lo siguiente para ejecutar el ejercicio 3:\n ./arqo3 <tamanio_de_la_matriz> <forma_de_operar>\n");
	}

	tam = atoi(argv[1]);
	forma_de_operar = atoi(argv[2]);

	if(!forma_de_operar){//Operamos de forma normal en tal caso
		tiempo = multiplyMatrix(tam);
	}

	if(forma_de_operar){//Operamos de forma traspuesta en tal caso
